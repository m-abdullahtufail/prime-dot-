import Reveal from "@/components/Reveal";
import { getContent } from "@/lib/content";

export default function TeamPreview() {
  const { teamSection, team } = getContent();
  return (
    <section className="relative py-24 md:py-32">
      <div className="mx-auto w-full max-w-7xl px-6 lg:px-8">
        <Reveal>
          <div className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
            <div>
              <p className="kicker">
                <span className="nav-dot mr-2" />
                {teamSection.kicker}
              </p>
              <h2 className="mt-5 font-display text-3xl font-bold uppercase leading-tight tracking-tight md:text-5xl">
                {teamSection.title.map((line) => (
                  <span key={line.text} className="block">
                    <span className={line.accent ? "text-shine" : ""}>
                      {line.text}
                    </span>
                  </span>
                ))}
              </h2>
            </div>
            <a
              href="/team"
              className="nav-link flex items-center gap-2 hover:text-prime-light"
            >
              {teamSection.viewAllLabel}
            </a>
          </div>
        </Reveal>

        <div className="mt-14 grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {team.map((member, i) => (
            <Reveal key={member.name} delay={i * 110}>
              <article className="card group flex h-full items-center gap-5 p-6">
                <div className="relative shrink-0">
                  <div className="absolute inset-0 rounded-full bg-prime/30 blur-lg transition-all duration-500 group-hover:bg-prime/60" />
                  <div className="relative grid size-16 place-items-center rounded-full border border-prime/40 bg-gradient-to-b from-ink-soft to-ink font-display text-lg font-bold text-cream transition-transform duration-500 group-hover:scale-105">
                    {member.initials}
                  </div>
                </div>
                <div>
                  <h3 className="font-display text-sm font-bold uppercase tracking-wide text-cream">
                    {member.name}
                  </h3>
                  <p className="mt-1 font-mono text-[10px] uppercase tracking-[0.2em] text-prime-light">
                    {member.role}
                  </p>
                </div>
              </article>
            </Reveal>
          ))}
        </div>
      </div>
    </section>
  );
}
